...
class ControllerCatalogCategory extends Controller { 
	...
	protected function getForm() {
        ...
        $category_parent_ids = false;

		if (isset($this->request->get['category_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$category_info = $this->model_catalog_category->getCategory($this->request->get['category_id']);
            $category_parent_ids = $this->model_catalog_category->getCategoryParentIds($this->request->get['category_id']);
            $category_parent_ids = explode(':',$category_parent_ids['path']);
    	}
		...
		if (isset($this->request->post['keyword'])) {
			...
			$this->data['keyword'] = $this->model_catalog_category->getCategoryKeywords($this->request->get['category_id']);
		} else {
			$this->data['keyword'] = '';
		}
        ...
        if (isset($this->request->post['category_quantity_discount'])) {
            $this->data['category_quantity_discount'] = $this->request->post['category_quantity_discount'];
        } elseif (isset($this->request->get['category_id'])) {
            $this->data['category_quantity_discount'] = $this->model_catalog_category_quantity_discount->getDiscountByCategoryId($this->request->get['category_id']);
        } else {
            $this->data['category_quantity_discount'] = array();
        }

        /*
         * jeśli kategoria nie posiada własnych zniżek ilościowych to korzystamy z ustawień nadrzędnej kategorii lub ustawień ogólnych modułu
         */
        if(empty($this->data['category_quantity_discount']))
        {
            // get parent category settings ( recursively ) , if not get module settings
            $category_quantity_discount = false;
            if($category_parent_ids)
            {
                foreach($category_parent_ids as $parent_id)
                {
                    $category_quantity_discount = $this->model_catalog_category_quantity_discount->getDiscountByCategoryId($parent_id);
                    if(!empty($category_quantity_discount))
                    {
                        break;
                    }
                }
            ...
            {
                $this->data['category_quantity_discount'] = $this->config->get('quantity_discount_values');
            }
            else
            {
                $this->data['category_quantity_discount'] = $category_quantity_discount;
            }
        ...
        // rodzaje filtrów
        $this->data['extended_filter_types'] = array(
            'price',
            'option',
            'attribute',
        );
        ...
        // filtry przypisane do kategorii
        if (isset($this->request->post['category_extended_filter'])) {
            $this->data['category_extended_filter'] = $this->request->post['category_extended_filter'];
        } elseif (isset($this->request->get['category_id'])) {

            $this->data['category_extended_filter'] = $this->model_catalog_extended_filter->getExtendedFiltersByCategory($this->request->get['category_id']);
            // jak puste to bierzemy od parenta
            if(empty($this->data['category_extended_filter']) AND isset($category_info))
            {
                $this->data['category_extended_filter'] = $this->model_catalog_extended_filter->getExtendedFiltersByCategory($category_info['parent_id']);
            }
        } else {
            $this->data['category_extended_filter'] = array();
        }
