...
class ControllerCatalogManufacturer extends Controller {
    ...
    protected function getList() {
        ...
        $manufacturer_groups = $this->model_catalog_manufacturer_group->getManufacturerGroups();

        $this->data['manufacturer_groups'] =  array();

        $this->data['manufacturer_groups'][0] = array(
            'name' => 'Pozostałe',
            'manufacturers' => array(),
        );

        foreach($manufacturer_groups as $manufacturer_group)
        {
            $this->data['manufacturer_groups'][$manufacturer_group['manufacturer_group_id']] = array(
                'name' => $manufacturer_group['name'],
                'manufacturers' => array(),
            );
        }
        ...
        foreach ($results as $result) {
            ...
            $this->data['manufacturer_groups'][$result['manufacturer_group']]['manufacturers'][] =  array(
                'manufacturer_id' => $result['manufacturer_id'],
                'name'            => $result['name'],
                'sort_order'      => $result['sort_order'],
                'selected'        => isset($this->request->post['selected']) && in_array($result['manufacturer_id'], $this->request->post['selected']),
                'action'          => $action,
                'manufacturer_group' => $result['manufacturer_group'],
            );
        ...
        $this->data['manufacturer_groups'] = array_reverse($this->data['manufacturer_groups']);
    ...
    protected function getForm() {
        ...
        if (isset($this->request->post['description'])) {
            ...
            $this->data['description'] = $manufacturer_info['manu_description'];
        } else {
            $this->data['description'] = '';
        }

        if (isset($this->request->post['follow'])) {
            $this->data['follow'] = $this->request->post['follow'];
        } elseif (!empty($manufacturer_info)) {
            $this->data['follow'] = $manufacturer_info['follow'];
        } else {
            $this->data['follow'] = '';
        }

        if (isset($this->request->post['manufacturer_group'])) {
            $this->data['manufacturer_group'] = $this->request->post['manufacturer_group'];
        } elseif (!empty($manufacturer_info)) {
            $this->data['manufacturer_group'] = $manufacturer_info['manufacturer_group'];
        } else {
            $this->data['manufacturer_group'] = 0;
        }

        $this->load->model('catalog/manufacturer_group');

        $this->data['manufacturer_groups'] = $this->model_catalog_manufacturer_group->getManufacturerGroups();
