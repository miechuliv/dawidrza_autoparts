...
class ControllerSaleOrder extends Controller {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$this->data['products'][] = array(
				    ...
				    'kaucja' => $product['kaucja_cost']!=0?($product['kaucja'].': '.$this->currency->format($product['kaucja_cost'])):false,
                );
			}
            ...
            // miechu sms
            if (isset($this->request->post['send_sms'])) {
                $this->data['send_sms'] = $this->request->post['send_sms'];
            } else {
                $this->data['send_sms'] = $this->config->get('config_smsapi_update');
            }

            // miechu sms
            if (isset($this->request->post['sms_message'])) {
                $this->data['sms_message'] = $this->request->post['sms_message'];
            } else {
                $this->data['sms_message'] = $this->config->get('config_smsapi_order_update_msg');
            }
	...
	public function history() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if (strlen($this->request->post['sms_message']) > 160) {
                $this->data['error'] = 'Przekroczono limit znaków wiadomości sms';
            }
