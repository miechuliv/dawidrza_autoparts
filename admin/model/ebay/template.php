...
class ModelEbayTemplate extends Model{
    ...
    public function get($id){
        ...
        if($qry->num_rows){
            $row                = $qry->row;
            $row['link_edit']   = HTTPS_SERVER . 'index.php?route=ebay/openbay/templateEdit&token=' . $this->session->data['token'].'&template_id='.$row['template_id'];
            $row['link_delete'] = HTTPS_SERVER . 'index.php?route=ebay/openbay/templateDelete&token=' . $this->session->data['token'].'&template_id='.$row['template_id'];
