...
class ControllerPaymentGiropayDirect extends Controller {
	protected function index() {
		$this->language->load('payment/giropay_direct');

		$this->data['text_title'] = $this->language->get('text_title');
		$this->data['text_wait'] = $this->language->get('text_wait');

		$this->data['entry_bankcode'] = $this->language->get('entry_bankcode');
		$this->data['entry_bankaccount'] = $this->language->get('entry_bankaccount');

		$this->data['button_confirm'] = $this->language->get('button_confirm');

		$this->data['action'] = 'https://payment.girosolution.de/payment/start';

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/giropay_direct.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/giropay_direct.tpl';
		} else {
			$this->template = 'default/template/payment/giropay_direct.tpl';
		}
	...
	public function getParameters() {
		...
		if ($order_info) {
			$parameters = array(
				...
				'projectId'     => $this->config->get('giropay_direct_project_id'),
				'bankcode'      => $bankcode,
				'bankaccount'   => $bankaccount
			);

			$parameters['hash'] = $this->hmac('md5', implode($parameters), htmlspecialchars_decode($this->config->get('giropay_direct_secret'), ENT_QUOTES));

			$url = 'https://payment.girosolution.de/payment/validate/bankaccount';
			...
			if ($result == 'sb200') {
				$parameters = array(
					...
					'vwz'           => utf8_substr(($this->config->get('giropay_direct_description') ? $this->config->get('giropay_direct_description') : $this->language->get('text_order') . ' ' . $this->session->data['order_id']), 0, 27, 'UTF-8'),
					'bankcode'      => $bankcode,
					'bankaccount'   => $bankaccount,
					'urlRedirect'   => $this->url->link('payment/giropay_direct/giropayRedirect'),
					'urlNotify'     => $this->url->link('payment/giropay_direct/giropayNotify')
				);
	...
	public function giropayNotify() {
		...
		if ($data) {
			...
			if ($data['gpCode'] == '4000') {
				$this->model_checkout_order->confirm($data['order_id'], $this->config->get('giropay_direct_order_status_id'));
			}
		} else {
	...
	private function validate() {
		...
		$data = $this->config->get('giropay_merchant_id') . $this->config->get('giropay_direct_project_id') . $order_id . $gpCode;
		$hash = $this->hmac('md5', $data , htmlspecialchars_decode($this->config->get('giropay_direct_secret'), ENT_QUOTES));
