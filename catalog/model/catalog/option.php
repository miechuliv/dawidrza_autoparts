...
class ModelCatalogOption extends Model {
    ...
    public function getOptionValuesByCategory($option_id,$category_id) {
        $option_value_data = array();

        $option_value_query = $this->db->query("SELECT DISTINCT ov.option_value_id, ovd.name, ov.image, ov.sort_order FROM " . DB_PREFIX . "option_value ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id)
         LEFT JOIN product_option_value pov ON(ov.option_value_id = pov.option_value_id)
         LEFT JOIN product p ON(pov.product_id = p.product_id)
