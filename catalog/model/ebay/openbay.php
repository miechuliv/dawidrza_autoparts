...
class ModelEbayOpenbay extends Model{
    ...
    private function create($order){
        ...
        $this->ebay->log('Order is being inserted with the date: '.$createdDate);
        $this->ebay->log('Original date was : '.(string)$order->order->created);
        $this->ebay->log('Offset was: '.$offset);
        $this->ebay->log('Server time is: '.date("Y-m-d H:i:s"));
        ...
        foreach ($order->txn as $txn) {
            ...
            if($product_id != false){
                $this->ebay->log('Product ID: "'.$product_id.'" from ebay item: '.$txn->item->id.' was returned');
                ...
                $this->ebay->log('No product ID from ebay item: '.$txn->item->id.' was returned');
                $model_number = '';
            }

            $qty            = (int)$txn->item->qty;
            $price          = (double)$txn->item->price;
            $this->ebay->log('Gross price: '.$price);

            /*
             * @todo
             * The tax needs to be calculated in this priority
             * 1. if eBay send a tax amount
             * 2. if the product is linked get the tax percentage
             * 3. if nothing else default to the module tax amount
             */
            
            $priceNet       = $price / $this->tax;
            $this->ebay->log('Net price: '.$priceNet);

            $totalNet       = $priceNet * $qty;
            $this->ebay->log('Total net price: '.$totalNet);

            $tax            = number_format(($price - $priceNet), 4,'.','');
            $this->ebay->log('Tax: '.$tax);

            $totalTax       = $tax * $qty;
            $this->ebay->log('Total Tax: '.$totalTax);
            ...
            $this->ebay->log('Added order product id '.$order_product_id);

            if($openstock == true){
                $this->ebay->log('OpenStock enabled');
    ...
    private function updateOrderWithConfirmedData($order_id, $order, $user){
        ...
        foreach ($order->txn as $txn) {
            $qty        = (int)$txn->item->qty;
            $price      = (double)$txn->item->price;
            $priceN     = $price / $this->tax;
            $total      = $price * $qty;
            $totalN     = $priceN * $qty;
            $tax        = number_format($price - $priceN, 4,'.','');
            $totalVat   += ($tax * $qty);
            $totalNet   += $totalN;
        }

        $discountnet    = (double)$order->order->discount / $this->tax;
        $discountvat    = (double)$order->order->discount - $discountnet;
        $shippingnet    = (double)$order->shipping->cost / $this->tax;
        $shippingvat    = (double)$order->shipping->cost - $shippingnet;
        $totalLangs     = $this->load->language('ebay/order');
        $data           = array();  
        
        $data['totals'][0] = array(
            'code'          => 'sub_total',
            'title'         => $totalLangs['lang_subtotal'],
        ...
        $data['totals'][1] = array(
            'code'          => 'shipping',
            'title'         => $totalLangs['lang_shipping'],
        ...
        $data['totals'][2] = array(
            'code'          => 'credit',
            'title'         => $totalLangs['lang_discount'],
        ...
        $tax = number_format($shippingvat + $totalVat + $discountvat, 4,'.','');

        $data['totals'][3] = array(
            'code'          => 'tax',
            'title'         => $totalLangs['lang_tax'],
