...
// http://www.altkomp.pl/
//-----------------------------------------
class ModelShippingKEXPLUS extends Model {
	private $name = '';
	...
	public function getQuote($address) {
        ...
        $this->load->language('shipping/KEXPLUS');
		...
		foreach ($geozones->rows as $result) {
   			if ($this->config->get('KEXPLUS_' . $result['geo_zone_id'] . '_status')) {
			...
			if ($status) {
				...
				$keyword = $this->config->get('KEX_' . $result['geo_zone_id'] . '_filter');
