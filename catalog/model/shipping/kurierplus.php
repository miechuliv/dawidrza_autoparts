<?php
//-----------------------------------------
// Shipping Kurierplus Opencart Module
// Version: 1.4.7 - 1.5.x
// Author: Altkomp PHU
// License: Commercial
...
// http://www.altkomp.pl/
//-----------------------------------------
class ModelShippingKurierplus extends Model {
	private $name = '';
	...
	public function getQuote($address) {
        ...
        $this->load->language('shipping/kurierplus');
		...
		foreach ($geozones->rows as $result) {
   			if ($this->config->get('kurierplus_' . $result['geo_zone_id'] . '_status')) {
			...
			if ($status) {
				...
				$keyword = $this->config->get('kurierplus_' . $result['geo_zone_id'] . '_filter');
