<?php
class ModelShippingUpsCod extends Model {
	function getQuote($address) {
		...
		if ($status) {
			...
			$xml .= '			</Address>'; 
			/// miechu negotatied rates
			$xml .= '           <ShipperNumber>E2389X</ShipperNumber>'; 
			...
			$xml .= '		</ShipFrom>'; 
				...
				// miechu negotiated 
			$xml .= '		<RateInformation>';  
			$xml .= '			<NegotiatedRatesIndicator>Account Based Rates';  
			...
			//	$xml .= '                   <COD>';
			//	$xml .= '                       <CODFundsCode>9</CODFundsCode>';
			 //   $xml .= '                       <CODAmount>100</CODAmount>';
		   // 	$xml .= '                   </COD>';
		  ...
		  //  var_dump($xml);
			...
			//	var_dump($response);
				...
				if ($response_status_code->item(0)->nodeValue != '1') {
					...
					//	var_dump($cost);
					//	var_dump($currency);
					//	var_dump($this->currency->convert($cost, $currency, $this->config->get('config_currency')));
						...
						if ($this->config->get('ups_' . strtolower($this->config->get('ups_origin')) . '_' . $code)) {
							$quote_data[$code] = array(
								...
								'text' => $this->currency->format($cost,$this->currency->getCode())
							);
						}
					}
				}
