...
class amazonus {
    ...
    //private $server = 'http://us-amazon.welfordlocal.co.uk/';
     ...
     */
     ...
     /*
     public function getLinkedId($amazonusSku) {
        return $this->db->query("SELECT `product_id`
            FROM `" . DB_PREFIX . "amazonus_product_link`
            WHERE `amazonus_sku` = '" . $this->db->escape($productId) . "'
            ")->row;
     }
     */
