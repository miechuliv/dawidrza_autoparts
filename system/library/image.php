...
class Image {
	...
	*/
    ...
    public function addwatermark($position = 'bottomright',$watermark=false) {

        if(!$watermark AND file_exists(DIR_IMAGE . 'watermark.png'))
        {
        ...
        {
            $output_file_name = DIR_IMAGE . "output.png";
            $img = imagecreatefromstring(file_get_contents(DIR_IMAGE .$watermark));
            ...
            imagefilter($img, IMG_FILTER_COLORIZE,0,0,0,90);

            imagepng($img, $output_file_name);

            $watermark = imagecreatefrompng($output_file_name);
